...
///
/// CursorPosToIndex
// convert the X/Y cursor position to an index value
LONG CursorPosToIndex(struct InstData *data)
{
  LONG idx = 0;
  struct line_node *line;
  ...
  // count the length of all lines up to the current one
  ...
  // add the cursor position within the current line
  idx += data->CPos_X;
...
///
/// IndexToCursorPos
// convert an index position to an X/Y cursor position
void IndexToCursorPos(struct InstData *data, LONG idx)
{
  struct line_node *line;
  LONG lineNumber = 0;
  ...
  // within that line.
  line = GetFirstLine(&data->linelist);
  while(line != NULL && idx > line->line.Length-1)
  {
    idx -= line->line.Length;
    lineNumber++;
    line = GetNextLine(line);
  ...
  SetAttrs(data->object,
    MUIA_TextEditor_CursorX, idx,
    MUIA_TextEditor_CursorY, lineNumber,
    TAG_DONE);
